...
function createFallingImage() {
    ...
    var imageWidth  = Math.floor(Math.random() * 400) + 50; // 50から150の間でランダムな幅を設定
    var imageHeight = Math.floor(Math.random() * 100) + 400; // 50から550の間でランダムな高さを設定
    
    image.style.left = Math.floor(Math.random() * (windowWidth + imageWidth)) - imageWidth + 'px'; // ランダムな水平位置を設定
    ...
    function fall() {
        ...
        if(posY > windowHeight - image.height + 20){
            speed = 0;
